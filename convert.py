...
def convert_images_to_avif(input_folder, output_file, fps=30, quality=85, progress_callback=None):
    """
    ...
    try:
        ...
        print(f"FPS: {fps}, Duration per frame: {duration}ms, Quality: {quality}")
...
def main():
    ...
    success = convert_images_to_avif(args.input_folder, args.output_file, args.fps, args.quality)
    if not success:
        sys.exit(1)
